...
public class MouseAndCheeseMaze {
    ...
    public MouseAndCheeseMaze(Maze maze, int mouseX, int mouseY, int cheeseX, int cheeseY) {
        ...
        if (!maze.getLocation(mouseX, mouseY).isOpen() || !maze.getLocation(cheeseX, cheeseY).isOpen()) {
            throw new IllegalArgumentException("Illegal rat/mouse or cheese coordinates: they must be within the maze and in open cells.");
        }
    ...
    /**
     *  Sets the maze walker.
     */
    public void setMazeWalker(MazeWalker mazeWalker) {
        this.mazeWalker = mazeWalker;
    }
    ...
    public void setMouseLocation(int mouseX, int mouseY) {
        Location proposedLocation = maze.getLocation(mouseX, mouseY);
        if (!proposedLocation.isOpen()) {
            throw new IllegalArgumentException("Mouse location (" + mouseX + ", " + mouseY + ") is not allowed!");
        }
        ...
        if (mouse != null) {
            ...
            if (mouseX == this.mouse.x) {
                if ((mouseY < this.mouse.y - 1) || (mouseY > this.mouse.y + 1)) {
                    throw new IllegalArgumentException("Mouse location (" + mouseX + ", " + mouseY + ") is not adjacent!");
                }
    ...
    public Result updateMazeState() {
        ...
        Location currentLocation = maze.getLocation(mouse.x, mouse.y);
        Location nextLocation = null;

        // Walk the maze.
        WalkerState walkerState = mazeWalker.areWeThereYet(mouse.x, mouse.y);
        switch (walkerState) {
            case THERE_ALREADY:
